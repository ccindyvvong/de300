...
class NumSquare(Transformer):
    ...
    def _transform(self, df: DataFrame) -> DataFrame:
        for in_col, out_col in zip(self.__inputCols, self.__outputCols):
            df = df.withColumn(out_col, col(in_col) * col(in_col))
...
def pipeline(data: DataFrame):
    ...
    # Squares of numeric features
    square_columns = [f"{col}_squared" for col in numeric_features]
    squares = NumSquare(inputCols=imputed_columns_numeric, outputCols=square_columns)

    # Assemble feature columns into a single feature vector
    assembler = VectorAssembler(
        inputCols=square_columns + pairwise_columns + imputed_columns_numeric + imputed_columns_string, 
        outputCol="features"
        )
    ...
    # Create the pipeline
    pipeline = Pipeline(stages=[indexer, *imputers_string, imputer_numeric, pairwise_product, squares, assembler, classifier])
